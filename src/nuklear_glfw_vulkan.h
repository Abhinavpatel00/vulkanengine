...
static const unsigned char nuklearshaders_nuklear_vert_spv[] = {
...
static const unsigned int nuklearshaders_nuklear_vert_spv_len = 1856;
static const unsigned char nuklearshaders_nuklear_frag_spv[] = {
    0x03, 0x02, 0x23, 0x07, 0x00, 0x00, 0x01, 0x00, 0x0b, 0x00, 0x0d, 0x00,
    ...
    0xfd, 0x00, 0x01, 0x00, 0x38, 0x00, 0x01, 0x00};
static const unsigned int nuklearshaders_nuklear_frag_spv_len = 860;
...
struct nk_glfw_device
{
	...
	/* Framebuffer size (swapchain extent) in pixels for current image_views */
...
NK_INTERN void nk_glfw3_create_pipeline(struct nk_glfw_device* dev)
{
	...
	memset(&pipeline_rendering_create_info, 0, sizeof(VkPipelineRenderingCreateInfoKHR));
	pipeline_rendering_create_info.sType = VK_STRUCTURE_TYPE_PIPELINE_RENDERING_CREATE_INFO_KHR;
	pipeline_rendering_create_info.colorAttachmentCount = 1;
	pipeline_rendering_create_info.pColorAttachmentFormats = &dev->color_format;
    ...
    uint32_t framebuffer_width, uint32_t framebuffer_height)
{
	...
	dev->framebuffer_width = framebuffer_width;
	dev->framebuffer_height = framebuffer_height;
    ...
    enum nk_anti_aliasing AA)
{
	...
	/* Use framebuffer pixel size to match swapchain image dimensions */
	projection.m[0] /= (float)dev->framebuffer_width;
	projection.m[5] /= (float)dev->framebuffer_height;
	...
	memset(&color_attachment_info, 0, sizeof(VkRenderingAttachmentInfo));
	color_attachment_info.sType = VK_STRUCTURE_TYPE_RENDERING_ATTACHMENT_INFO;
	color_attachment_info.imageView = dev->image_views[buffer_index];
	color_attachment_info.imageLayout = VK_IMAGE_LAYOUT_COLOR_ATTACHMENT_OPTIMAL;
	color_attachment_info.loadOp = VK_ATTACHMENT_LOAD_OP_LOAD;
	color_attachment_info.storeOp = VK_ATTACHMENT_STORE_OP_STORE;
	color_attachment_info.clearValue = clear_value;

	memset(&rendering_info, 0, sizeof(VkRenderingInfo));
	rendering_info.sType = VK_STRUCTURE_TYPE_RENDERING_INFO;
	/* Match renderArea to framebuffer size to satisfy VUID 06080 */
	rendering_info.renderArea.extent.width = dev->framebuffer_width;
	rendering_info.renderArea.extent.height = dev->framebuffer_height;
	...
	/* Viewport in pixels */
	viewport.width = (float)dev->framebuffer_width;
	viewport.height = (float)dev->framebuffer_height;
